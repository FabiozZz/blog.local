...
class PostController extends Controller
{
    ...
    {
        $posts = Post::query()->select('posts.*', 'users.name as author')
            ->join('users', 'posts.author_id', '=', 'users.id')
            ->orderBy('posts.created_at', 'desc')
            ->paginate(4);
    ...
    public function store(PostRequest $request, Post $post)
    {
        ...
        $post->except = $request->input('except');
    ...
    {
        $post = Post::query()->select('*', 'users.name as author')
            ->join('users', 'posts.author_id', '=', 'users.id')
            ->find($id);
        if (is_null($post)) {
            abort(404);
    ...
    private function uploadImage(PostRequest $request, Post $post)
    {
        ...
        if ($source) {
            // Перед загрузкой нового изображение удаляем старое
            $this->removeImage($post);
            /*
    ...
    {
        return Auth::id() == $post->id || Auth::id() == 1;
    }
}
